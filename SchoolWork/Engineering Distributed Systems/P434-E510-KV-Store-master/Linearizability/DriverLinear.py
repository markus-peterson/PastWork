...
for i in range(3):
    ...
    if i == 0:
        curr_dir = os.path.dirname(__file__)
        node_py = curr_dir + os.sep + "NodeLinear.py"
...
for i in range(10):
    ...
    print("as the set should be blocking. The gets also cause a multicast.")
    print("--------------------------------------------------------------------")
    value = str(random.randint(0, i))

    start = time.time()
    print(node2.set("test", value))
    stop = time.time()
    set_times.append((stop-start))

    start = time.time()
    print(f"Expects {value} got " + node1.get("test"))
    stop = time.time()
    get_times.append((stop - start))

    start = time.time()
    print(f"Expects {value} got " + node2.get("test"))
    stop = time.time()
    get_times.append((stop - start))

    start = time.time()
    print(f"Expects {value} got " + seq.get("test"))
    stop = time.time()
    get_times.append((stop - start))
